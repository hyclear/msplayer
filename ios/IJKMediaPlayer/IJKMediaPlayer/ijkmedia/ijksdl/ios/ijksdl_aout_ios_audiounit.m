...
static void aout_set_volume(SDL_Aout *aout, float left_volume, float right_volume)
{
    ...
    SDL_Aout_Opaque *opaque = aout->opaque;
    if (opaque) {
        [opaque->aoutController setVolume:left_volume];
    }
}

static float aout_get_volume(SDL_Aout *aout)
{
    if (!aout) {
        return 0.0;
    }
    ...
    SDL_Aout_Opaque *opaque = aout->opaque;
    if (opaque) {
        return [opaque->aoutController getVolume];
    }
...
SDL_Aout *SDL_AoutIos_CreateForAudioUnit()
{
    ...
    aout->get_volume  = aout_get_volume;
