...
static void vout_free_l(SDL_Vout *vout)
{
    ...
    if (opaque) {
        ...
        if (opaque->gl_view_array) {
            [opaque->gl_view_array removeAllObjects];
            [opaque->gl_view_array release];
            opaque->gl_view_array = nil;
        }
    }
...
static int vout_display_overlay_l(SDL_Vout *vout, SDL_VoutOverlay *overlay)
{
    ...
    for (int i = 0; i < opaque->gl_view_array.count; ++i){
        gl_view = [opaque->gl_view_array objectAtIndex:i];
...
static void SDL_VoutIos_AddGLView_l(SDL_Vout *vout, IJKSDLGLView *view)
{
    SDL_Vout_Opaque *opaque = vout->opaque;
    
    if (!opaque->gl_view_array) {
        opaque->gl_view_array = [[NSMutableArray alloc]init];
    }
    
    if (view) {
        [opaque->gl_view_array addObject:view];
        ALOGI("SDL_VoutIos_AddGLView_l view[%p], count[%ld]\n", view, (unsigned long)opaque->gl_view_array.count);
    }
}

static void SDL_VoutIos_removeGLView_l(SDL_Vout *vout, IJKSDLGLView *view)
{
    SDL_Vout_Opaque *opaque = vout->opaque;
    
    if (view) {
        [opaque->gl_view_array removeObject:view];
        ALOGI("SDL_VoutIos_removeGLView_l view[%p], count[%ld]\n", view, (unsigned long)opaque->gl_view_array.count);
    }
}
...
void SDL_VoutIos_AddGLView(SDL_Vout *vout, IJKSDLGLView *view)
{
    SDL_LockMutex(vout->mutex);
    SDL_VoutIos_AddGLView_l(vout, view);
    SDL_UnlockMutex(vout->mutex);
}

void SDL_VoutIos_RemoveGLView(SDL_Vout *vout, IJKSDLGLView *view)
{
    SDL_LockMutex(vout->mutex);
    SDL_VoutIos_removeGLView_l(vout, view);
    SDL_UnlockMutex(vout->mutex);
}
